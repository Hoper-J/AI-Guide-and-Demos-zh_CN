...
def transcribe_audio(audio_path, subtitle_path, decode_options, model_name="medium"):
    ...
    try:
        ...
        print(f"网络错误: {e}. 可能是网络连接或代理问题，请检查你的网络设置。")
    except ssl.SSLError as e:
        print(f"SSL 错误: {e}. 可能是网络连接或代理问题，请检查你的网络设置。")
    except Exception as e:
        print(f"音频转录时出错: {e}")
    return None
...
def read_subtitle(file_path, timestamped=False):
    ...
    try:
        ...
        print(f"读取字幕文件时发生未知错误: {e}")
    return None
    
def summarize_text(text, client, timestamped=False, model="qwen-vl-max-0809"):
    """
...
def process_file(file_path, client, output_dir=None, timestamped=False, model_name="medium", language="zh", temperature=0.2):
    ...
    - temperature (float): 生成摘要时的温度，默认为 0.2。
    ...
    summary = summarize_text(text, client, timestamped=timestamped)
    if summary is None:
        return "摘要生成失败"
...
def main():
    ...
    parser.add_argument("--model_name", type=str, choices=['tiny', 'base', 'small', 'medium', 'large-v3'],
                        default=config.get('model_name', 'medium'), help="指定使用的 Whisper 模型名称，medium 需要你拥有 5G 以上的显存")
    parser.add_argument("--language", type=str, default=config.get('language', 'zh'), help="音频转录的语言")
    parser.add_argument("--temperature", type=float, default=config.get('temperature', 0.2), help="生成文本时的温度，范围为 0 到 1")
    parser.add_argument("--timestamped", action="store_true", default=config.get('timestamped', False), help="让转录后的文本信息保留时间戳")
    
    args = parser.parse_args()

    # 获取 API 密钥：从命令行或配置文件中获取
    ...
    print(f"- API Base URL: {config.get('api_base_url', 'https://dashscope.aliyuncs.com/compatible-mode/v1')}")

    # 模型相关配置
    print("\n>> Whisper 相关配置 <<")
    print(f"- Whisper 模型: {args.model_name}")
    print(f"- 语言: {args.language}")
    print(f"- Temperature: {args.temperature}")
    
    # 其他配置
    print("\n>> 其他配置 <<")
    print(f"- 是否保留时间戳: {args.timestamped}")    
    print("\n=================================\n")

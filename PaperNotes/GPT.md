...
其实到此为止，还看不到现在的 ChatGPT 的影子，因为针对不同的任务还需要进行微调，不能简单的直接用对话的形式获取答案，...
...
>
> [GPT，GPT-2，GPT-3 论文精读【论文精读】 33:12 - 46:05](https://www.bilibili.com/video/BV1AF411b7xQ/?share_source=copy_web&vd_source=40b3e12ca72bba004f5dd21c08776797&t=1992)
...
3. **零样本学习（Zero-Shot Learning）**
   ...
   GPT-2 的**创新**在于对零样本学习的进一步探索。GPT-1 微调时引入了三种特殊符号：$\langle s \rangle$, $\$$, $\langle e \rangle$，用于分别表示开始，分割和结束，GPT-2 不再引入这些特殊符号，采用与 GPT-1 预训练数据格式更相似的自然输入格式（其实就是不做多余操作），这也是后续文献常提及以及我们现在耳熟能详的 `prompt`，作者给出了两个例子：

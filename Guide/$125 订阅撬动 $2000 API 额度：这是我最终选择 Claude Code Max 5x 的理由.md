...
> [!important]
> 本文发布于 2025-09-02。截止 2026-04-13，文中的一些模型表述已过时，本文保留仅作当时体验分享，最新建议见：[Claude / GPT 订阅建议与反代避坑](./Claude%20和%20GPT%20订阅建议与反代避坑.md)。
...
> [!note]
> 以下内容已按 2026-04-13 的使用体验更新。

- **用量不大 / 预算有限（\$20）/ 非代码场景**：建议尝试 ChatGPT Plus。
- **熟悉业务，能看懂代码，追求开发响应速度，且热衷于 review / 重构**：Claude。Claude Code 的 CLI 产品设计、长上下文协作和代码审阅体验，依旧是个人感觉最舒服的。
  - **做大规模开发**：Claude Max 计划，最近实测下来，Max20x 一周大概有 1400 刀的用量（因账号而异）。
  - Max 计划建议安装类似于 `claude-monitor` 的项目实时观测消耗，这会大幅增加你的开发速度和使用量（每一次会话的进度条都会提醒你“浪费”了多少额度）（之前的 claude-monitor 的作者在并没有继续更新，所以它对 Opus 的费用估算会比官网实际价格高约 2 倍，更合适的替代品是 [ccusage](https://github.com/ryoppippi/ccusage)）。
- **对于最近一年比较新的开源项目 / 协议的开发**：GPT。如果阅读过技术文档且本身有一定了解，可以选择 Claude。
- **通用知识学习、文章撰写、习惯图形界面、完全 Vibe Coding**：GPT。
- **科研论文与比赛**：两家都可以。预算提升到 `\$100` 及以上时，更推荐 GPT，GPT Pro 会给你带来惊喜。
- **有生图的需求，或者需要 Google 的一些生态**：可以考虑 Gemini。

如果你是一个“新手”，请直接选择 ChatGPT Plus。只有当你真正知道自己的开发需求，能够看懂生成的代码，明白 AI 只是工具而非权威的时候，Max/Pro 这个价位带来的高额度才会成为助力。记住，掌控方向的始终需要是你自己。

**End**：AI 订阅确实非常贵，这是事实。但它省下的时间更加珍贵，这才是订阅它的真正原因。

...
**INT8** 的取值范围是 $[-128, 127]$，总共有 **256** 个可能的值。
...
以下代码展示了量化和反量化的操作实力，包含使用PyTorch实现、根据之前公式自定义，以及模型量化的操作。
...
你需要注意到，在这里我们**取巧**地使得 `abs(max(fp32_values))==abs(min(fp32_values))==6.0`，并且`mean(fp32_values)==0`，从而向你展示了完全不同的两种量化方式，尽管当前结果看起来是一致的。

为什么说取巧呢？尝试将`fp32_values`分别定义为另一种形式: 
...
你可能会注意到 **127.5** 这个奇怪的数值，[PyTorch源码](https://github.com/pytorch/pytorch/blob/55c343fa3a49cd6b8a774e8419f7843063e2a1ea/torch/ao/quantization/observer.py#L362)或许能够解决你的疑惑，完整源码见附录部分: 

# 关于 Claude Code 对第三方路径的隐写行为以及可能的解决方案
...
其实在之前“开源”的 2.1.88 源码中就有检测 ANTHROPIC_BASE_URL 的行为了，当时还会检测模型是否为 ^claude-(opus|sonnet)-4-6 来决定用户能否开启 auto mode。所以还蛮巧的，之前正好做了个伪装为第一方请求的工具，已经整理[开放](https://github.com/Hoper-J/ccwrap/blob/main/README.md)。原本是为了第三方用 auto mode 而开发，但由于隐写基于 `isFirstParty()` 进行判断，所以能够避开当前的特殊处理：
...
![ccwrap 抓包：伪装后日期行为 ASCII 撇号加连字符（无水印）](./assets/20260701175111647.png)
...
实际上如果是第三方路径，Claude Code 的请求头还会去掉一些特性，比如：`advanced-tool-use`，同时 mcp 会变成全部内联占用大量 token 而非使用 `ToolSearch` 工具延迟加载，这可能会导致首请求多出八成左右的 token：
...
下面的动画基于 2.1.193 版本，在 `claude -p hi` 下分别捕捉官方订阅账户与直接使用第三方的请求后制作，可以看到第三方的首请求甚至会多出 82% 的 token：

![动画：第三方路径请求体+82%，工具 schema 全内联](./assets/20260705233610914.gif)

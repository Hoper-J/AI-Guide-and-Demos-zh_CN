...
> 半年来在 Claude 和 GPT 的 CLI 上累计消耗了约 60,000\$ 用量的 tokens（Claude : GPT = 5: 1）。这期间关于模型对比体验的文章断断续续其实写了不少，但 Agent 相关技术和概念迭代太快，新技术的学习、业务的跟进，再加上拖更（罪魁祸首），最后很多稿子都拖成了废稿。“痛定思痛”，赶在 GPT 下个版本前把这篇小分享补上。
>
> **最近一次更新时间**：2026-09-25，此时国外最新的模型参考组为：Fable 5.1｜Opus 5.5｜GPT 6，主要针对 Claude 和 GPT 进行对比推荐。
>
...
下表是我在实际体验后给出的订阅建议，可以根据自身实际情况参考，希望能够对你有所帮助（**2026-09-25 补充**：时至今日，GPT 6 已经足以应对学习期间所有的需求，所以下方标注「Claude / GPT」的场景，选择你最方便获取的订阅即可。如果都可以订阅，则更建议选择 Claude）：
...
| 科研论文 & 比赛 | Claude / GPT | 考虑到 Claude 封号力度和 GPT 6 Astra 的能力提升，当前二选一的场景都更推荐 GPT。但如果能够轻松订阅 Claude，可以选择它。 |
...
| 习惯图形界面而非命令行界面 | Claude / GPT |  |
| 预算有限（20\$）/ **新手** | Claude / GPT |  |
| 建模 | Claude / GPT |  |
| 画图 | GPT | gpt-image 效果极其惊艳 |
...
2026-04-10，GPT 同样推出了 100\$ 档的订阅服务。和 Claude Max 5x 一样，官方声明它在 Codex 上的用量为 Plus（20\$）的 5 倍，不过在 2026-05-31 前，这个倍率会临时翻到 10 倍（目前已经过时）：

![GPT Codex 用量倍率说明](../assets/gpt-codex-usage-multiplier.png)

**Q：目前 Claude Max 20x 和 GPT Pro 200\$ 的计划每周（7d）大概能用多少刀？**

保守测算下来，Claude 在 2500\$ 以上，GPT 在 1500\$ 左右。如果开发量足够大，订阅的收益会明显高于中转站点。

> **最近一次的 7d 窗口参考**
>
> - Claude Max 20x：周限额消耗 41% 时，使用约 1493\$。线性外推理论上限约 3641.5\$。
>   - 使用 `npx ccusage daily` 测算。
> - GPT Pro 200\$：周限额消耗 100% 时，使用约 1448.89\$。
>   - 使用 `npx @ccusage/codex daily` 测算。
>
> **注**：窗口的用量上限同样取决于消息数，或者说个人的使用习惯。早期在 578,979 tokens 的代码仓库下测试 Claude 8 Opus subagent 并行代码探索时，仅 20\$ 的用量就消耗了 20% 的 5h 限额和 2% 的 7d 限额，此时计算周限约 1000\$。

**Q：更轻量的 Claude Max 5x 够我的日常开发使用吗？GPT Pro 100\$ 呢？**

Claude Max 5x 用于日常开发肯定是够的，但如果你一整天都在编程，且喜欢使用 Plan / Spec / Subagents / 多窗口并行开发不停歇，那么 5x 用起来会有点拘束。在去年用量未下调的时候，Max 5x 的 5h 窗口用量也只有 50\$ 左右，当时大概会在 2 小时触达上限（有 Opus 4.1 价格比较贵的因素，是现在 Opus 5 价格的 3 倍）。

GPT Pro 100\$ 目前不再拥有 10x Plus 的用量，不支持高强度并发的编程开发。

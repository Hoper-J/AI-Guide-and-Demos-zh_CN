...
你应该能发现，这其中有很多非常熟悉的词：`messages`、`role`、`user` 和 `assistant`（在[最早](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/02.%20简单入门：通过API与Gradio构建AI应用.md#第一部分文章摘要单轮对话应用)调用 API 时其实见到过），整理格式进行理解：

...
>
> 在之前的文章[《06. 开始实践：部署你的第一个语言模型》](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/06.%20开始实践：部署你的第一个语言模型.md)中，我们展示了如何使用一个较小的预训练模型来部署对话应用，旨在为初学者提供一个简单的入门教程。但如果你想基于那篇文章，真正部署更流行的大模型，可能会遇到：
>
...
>
> 代码文件下载：[Transformers](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Demos/16a.%20使用%20Transformers%20加载量化后的%20LLM%20大模型（GPTQ%20%26%20AWQ）.ipynb) | [🎡 AI Chat 脚本](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/CodePlayground/chat.py)
...
> ```
>
> 在[《02. 简单入门：通过 API 与 Gradio 构建 AI 应用》](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/02.%20简单入门：通过%20API%20与%20Gradio%20构建%20AI%20应用.md#第一部分文章摘要单轮对话应用)的第一部分，你应该见过它的使用。
>
...
你应该能发现，这其中有很多非常熟悉的词：`messages`、`role`、`user` 和 `assistant`（在[最早](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/02.%20简单入门：通过%20API%20与%20Gradio%20构建%20AI%20应用.md#第一部分文章摘要单轮对话应用)调用 API 时其实见到过），整理格式进行理解：
...
在项目初期认识 API 的时候，文章[《01. 初识%20LLM API：环境配置与多轮对话演示》](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/01.%20初识%20LLM%20API：环境配置与多轮对话演示.md#流式输出)有提到过流式输出，这是我们一直以来见到的大模型输出方式：逐字（token）打印而非等全部生成完再打印。

...
另外，你还可以看到方法 1 的模型架构会多一个 base_model.model 包裹，如果你感兴趣的话可以使用 `print(model1)` 进一步地查看，这证明了在加载 LoRA 之前使用 `get_peft_model()` 会干扰模型结构，导致 LoRA 应用失效。但这其实不应该出现这种问题，我已经提了 issue，希望后续版本会解决。

...
> 是不是感觉很熟悉？Beam Search 是生成任务中常用的一种方法。在之前的文章 [05](../Guide/05.%20理解%20Hugging%20Face%20的%20%60AutoModel%60%20系列：不同任务的自动模型加载类.md)，[06](../Guide/06.%20开始实践：部署你的第一个语言模型.md)，[08](../Guide/08.%20尝试微调%20LLM：让它会写唐诗.md) 中，我们都曾经用到过与它相关的参数，而对于早就有着实操经验的同学们，想必见到的更多。这篇文章将从示例到数学原理和代码带你进行理解。
> 
...
> 
>在线链接：[Kaggle](https://www.kaggle.com/code/aidemos/07-beam-search) | [Colab](https://colab.research.google.com/drive/1majmMl7z3d7wGBCP4Zy-7g0zfMdZ_G1A?usp=sharing)
...
- **`max_length`（最大生成长度）**：限制生成序列的最大长度，注意该长度包含输入部分，现在更推荐使用 `max_new_tokens` 直接限制新生成的 token 数。
- **`length_penalty`（长度惩罚）**：用于调整生成序列的长度偏好，作为指数作用于序列长度，候选序列的得分会除以序列长度的该次幂。由于得分是序列的对数似然（为负值），取值大于 0 时会促进生成更长的序列，小于 0 时鼓励生成较短的序列，默认为 1.0。
...
with torch.no_grad():
    outputs = model.generate(
        ...
        early_stopping=True,  # 当有 num_beams 个候选序列生成<eos>时提前停止（除了 False 外还可以设置为 "never"）

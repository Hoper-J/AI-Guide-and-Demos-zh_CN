...
> 官方文档并未直接提供 `openai` 库的代码版本，不过通过观察源码可以发现绝大多数功能直接继承自 openai 库，所以接下来的行为非常简单，基本与阿里的使用方式一致，只需要对应修改以下三个参数：
>
> 1. api_key：《[00. 大模型 API 获取步骤](./00. 大模型 API 获取步骤.md#智谱-api)》。
...
```python
import os
os.environ['OPENAI_API_KEY'] = 'your-api-key' # 1
```

...
- [常见问题及解决方案](#常见问题及解决方案)
  - [问题一：RuntimeError 解决方法](#问题一runtimeerror--failed-to-import-transformersmodelsgpt2modeling_gpt2-because-of-the-following-error)
  - [问题二：TypeError 解决方法](#问题二typeerror-dispatch_model-got-an-unexpected-keyword-argument-offload_index)
- [总结](#总结)
- [附录](#附录)
- [参考链接](#参考链接)
...
1. **PyTorch**
    ...
    PyTorch 提供了 `torch.amp` 模块，可以方便地实现混合精度训练，加速计算并降低显存占用（`from torch.amp import GradScaler, autocast` 需要 PyTorch >= 2.3，更早版本需从 `torch.cuda.amp` 导入，该旧接口自 2.4 起已弃用）。
    ...
    for data, labels in dataloader:
        ...
        optimizer.zero_grad()
        with autocast('cuda'):  # 启用混合精度，默认 FP16
            outputs = model(data)
...
1. **PyTorch**
   ...
   ```python
   with autocast('cuda', dtype=torch.bfloat16):  # 手动启用 BF16
       outputs = model(data)
       loss = criterion(outputs, labels)
   ...
   ```
...
Training step memory usage: 629.14 MB
[Training Epoch 1, Batch 1] Allocated: 833.57 MB
...
Memory during training: 833.57 MB
Memory during inference: 221.48 MB
```

![Adam](./assets/Adam.png)

可以看到显存占比（加载：训练：推理）约为 1 : 4 : 1。

# a. **从部署到对话：使用 Transformers 本地运行量化 LLM 大模型（GPTQ & AWQ）**

> 来用点主流的大型语言模型（LLM）。
>
> LLM 的部署、微调和应用涉及多个方面，今天我们先聚焦于**部署**，本文的难点仅在于正确安装和知晓模型量化的概念 :)，所以不用担心，这是一篇轻松的文章。
>
...
> - ...
>
> 这篇文章会解决这些问题，并通过使用 Transformers 和 Llama-cpp-python 分别进行部署演示，帮助你更好地理解和掌握，本文对应于 Transformers，[19b](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/19b.%20从部署到对话：使用%20Llama-cpp-python%20本地运行量化LLM%20大模型（GGUF）.md) 对应于 Llama-cpp-python。
>
> 注意：当前文章对显卡没有强制要求，无显卡一样可以进行部署（推理使用 CPU ）。
>
> 与之前的文章不同是，为了直观感受，本文没有对代码进行函数封装，而是在需要时直接复用。
>
> [代码文件下载 - Transformers](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Demos/16a.%20使用%20Transformers%20部署量化后的%20LLM%20大模型（GPTQ%20%26%20AWQ）.ipynb)
...
## 目录
   ...
   - [手动下载模型（推荐）](#手动下载模型推荐)
     ...
     - [GGUF](#gguf)
     - [模型所在地](#模型所在地)
   - [使用 Transformers 开始部署](#使用-transformers-开始部署)
     - [环境配置](#环境配置)
     - [GPTQ](#gptq-1)
       - [安装](#安装)
       - [导入库](#导入库)
...
>
> Llama-cpp-python 的使用将位于文章 [19b](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/19b.%20从部署到对话：使用%20Llama-cpp-python%20本地运行量化LLM%20大模型（GGUF）.md)，建议在阅读完模型下载后再进行跳转。
>
...
如果你对 GGUF 文件的部署或者 Llama-cpp-python 的使用感兴趣，继续阅读 [19b](https://github.com/Hoper-J/AI-Guide-and-Demos-zh_CN/blob/master/Guide/19b.%20从部署到对话：使用%20Llama-cpp-python%20本地运行量化LLM%20大模型（GGUF）.md)。

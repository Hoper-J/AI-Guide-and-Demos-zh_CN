...
> 本文将使用 Transformers 和 LangChain，选择在 `Chinese` -> `Retrieval` 中表现较好的编码模型进行演示，这里选择 `tencent/Youtu-Embedding`。
>
> ![image-20260518101005761](./assets/image-20260518101005761.png)
>
...
# 打印生成的嵌入向量的长度，向量长度与模型的输出维度一致，你也可以选择打印向量看看
...
```sql
model_name='tencent/Youtu-Embedding' cache_folder=None model_kwargs={'trust_remote_code': True, 'model_kwargs': {'dtype': 'float16'}} encode_kwargs={} query_encode_kwargs={} multi_process=False show_progress=False
嵌入向量的维度为: 2048
```

可以看到当前编码模型最终输出的维度是 2048。
...
这里我们选择 `Qwen/Qwen2.5-1.5B-Instruct` 进行演示，当然，你可以替换它：

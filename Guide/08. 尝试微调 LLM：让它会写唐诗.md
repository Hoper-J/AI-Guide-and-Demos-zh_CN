...
>
> 在线链接：[Kaggle](https://www.kaggle.com/code/aidemos/06-llm) | [Colab](https://colab.research.google.com/drive/1It-LQynUU-UZCgT_N5T-_vw-u7N4fJB3?usp=sharing)
...
- `fsspec`：文件系统规范库。
- `trl`：Transformer Reinforcement Learning，提供 SFTTrainer 等训练工具。
...
def generate_training_data(data_point):
    ...
    返回:
        包含模型输入 IDs、注意力掩码和 completion_mask 的字典
...
[/INST]"""
    ...
    # tokenize 全文（不做 padding，交给 DataCollator 动态补齐）
    ...
    # completion_mask: 0 表示 prompt（不计算 loss），1 表示 output（计算 loss）
...
- **处理步骤**：
  ...
  - 单独对 `prompt` 进行 tokenize，获取其真实的 token 长度（不做 padding）。
  - 将完整的输入（提示词 + 输出）进行 tokenize。
  - 构建 `completion_mask`，对于提示词部分标记为 `0`（不计算 loss），对于输出部分标记为 `1`（计算 loss）。后续 TRL 的 `DataCollatorForLanguageModeling` 会根据 `completion_mask` 将提示词位置的 labels 设为 `-100`。
...
# 使用 TRL 的 SFTTrainer 进行模型训练
# 搭配 TRL 的 DataCollatorForLanguageModeling，通过 completion_mask 实现只在 output 部分计算 loss
# 注意：不能使用 transformers.Trainer，因为它会移除模型 forward 签名之外的列（如 completion_mask）
# SFTTrainer 重写了 _set_signature_columns_if_needed，显式保留 completion_mask 列
trainer = SFTTrainer(
    model=model,
